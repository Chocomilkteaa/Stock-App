...
import { MonthlyRevenueModel, MonthlyRevenueData } from '../models/MonthlyRevenue.model.js';
...
export class MonthlyRevenueService {
  private static parseData(decodedHtml: string): MonthlyRevenueData[] {
      ...
      .each((_, table) => {
        $(table)
          ...
          .each((__, row) => {
            ...
            rowObject.cumulativePreviousPeriodChangePercent = parseNum($(tds[9]).text());
  ...
  private static async fetchFromSource(
    ...
    try {
      ...
      return this.parseData(decodedHtml);
    } catch (error) {
      log(`[MonthlyRevenueService] Fetch ${type} error: ${(error as Error).message}`);
      return [];
    }
  }

  static async fetchMonthlyRevenues(dateStr: string): Promise<MonthlyRevenueData[]> {
    // dateStr format: YYYY-MM
    // 1. Check DB
    const existingData = await MonthlyRevenueModel.getMonthlyRevenuesByDate(dateStr);
    if (existingData.length > 0) {
      log(`[MonthlyRevenueService] Check DB: Found ${existingData.length} records.`);
      return existingData;
    }

    // 2. Crawl
    log(`[MonthlyRevenueService] Check DB: No data. Starting crawl job for ${dateStr}`);
    ...
    const [siiData, otcData] = await Promise.all([
      this.fetchFromSource(rocYear, month, 'sii'),
      this.fetchFromSource(rocYear, month, 'otc'),
    ]);
    ...
    // 3. Save to DB
    await MonthlyRevenueModel.saveMonthlyRevenues(allData, dateStr);
