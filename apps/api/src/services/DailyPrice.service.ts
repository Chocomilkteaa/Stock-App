...
async function fetchTwse({
  ...
  month,
  day,
}: DataDate): Promise<DailyPriceData[]> {
...
export async function fetchDailyPrices(
  ...
  const existingData = await getDailyPricesByDate(dateStr);
  ...
  if (allData.length === 0) {
    ...
    throw new DataNotFoundException(
      `No daily price data found for ${dateStr}. The market may have been closed or the date is invalid.`
    );
  }

  // 3. Save to DB
  await saveStocksAndPrices(allData, dateStr);
